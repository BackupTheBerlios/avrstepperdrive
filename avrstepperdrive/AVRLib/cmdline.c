...
void cmdlineAddCommand(u08* newCmdString, CmdlineFuncPtrType newCmdFuncPtr)
{
	...
	strcpy((char *)CmdlineCommandList[CmdlineNumCommands], (const char *)newCmdString);
...
{
	char* endptr;
	return strtol((const char *) cmdlineGetArgStr(argnum), (char **) &endptr, 10);
}
...
{
	char* endptr;
	return strtol((const char *)cmdlineGetArgStr(argnum), (char **)&endptr, 16);
}
